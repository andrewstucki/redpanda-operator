...
func TestReconcileTopic(t *testing.T) { // nolint:funlen // These tests have clear subtests.
	...
	t.Run("create_topic", func(t *testing.T) {
		...
		createTopic := v1alpha2.Topic{
			ObjectMeta: metav1.ObjectMeta{
				Name:      topicName,
				Namespace: testNamespace,
			},
			Spec: v1alpha2.TopicSpec{
				Partitions:        ptr.To(3),
		...
		assert.Equal(t, v1alpha2.ReadyCondition, cond.Type)
		assert.Equal(t, metav1.ConditionTrue, cond.Status)
		assert.Equal(t, v1alpha2.SucceededReason, cond.Reason)
	...
	t.Run("add_partition", func(t *testing.T) {
		...
		partitionCountChange := v1alpha2.Topic{
			ObjectMeta: metav1.ObjectMeta{
				Name:      topicName,
				Namespace: testNamespace,
			},
	...
	t.Run("unable_to_remove_partition", func(t *testing.T) {
		...
		assert.Equal(t, v1alpha2.FailedReason, cond.Reason)
		assert.NotEqual(t, 0, len(partitionCountChange.Status.TopicConfiguration))
	})
	t.Run("unable_to_increase_replication_factor", func(t *testing.T) {
		...
		// given topic custom resource
		replicationFactorChange := v1alpha2.Topic{
			ObjectMeta: metav1.ObjectMeta{
				Name:      topicName,
				Namespace: testNamespace,
			},
	...
	t.Run("both_tiered_storage_property", func(t *testing.T) {
		...
		tieredStorageConf := v1alpha2.Topic{
			ObjectMeta: metav1.ObjectMeta{
	...
	t.Run("delete_none_existent_topic_k8s_meta_deletion_timestamp", func(t *testing.T) {
		...
		noneExistentTestTopic := v1alpha2.Topic{
			ObjectMeta: metav1.ObjectMeta{
